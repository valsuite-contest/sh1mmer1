...
notice_and_self_shell() {
	echo "Run 'exec m1nsh1m_switch_root' to finish booting Sh1mmer."
	self_shell
}
...
echo "https://github.com/MercuryWorkshop/m1nsh1m"
echo ""
...
echo "[n] now to skip m1nsh1m patch (boot \"raw\" shim)"
echo ""
...
SKIP_M1NSH1M_PATCH=0
if [ "$(poll_key)" = "n" ]; then
	SKIP_M1NSH1M_PATCH=1
	echo "SKIPPING M1NSH1M PATCH"
	echo ""
fi
...
[ "$SKIP_M1NSH1M_PATCH" -eq 0 ] && patch_new_root_m1nsh1m
printf "${COLOR_RESET}"
echo ""

if [ "$SKIP_M1NSH1M_PATCH" -eq 0 ]; then
...
cat <<EOF >/bin/m1nsh1m_switch_root
#!/bin/busybox sh
...
EOF
chmod +x /bin/m1nsh1m_switch_root

[ "$(poll_key)" = "d" ] && notice_and_self_shell

enable_input
exec m1nsh1m_switch_root || :
