...
credits() {
	...
	echo "@olyb - Scraped additional shims & last remaining m1nsh1m maintainer"
...
mkdir -p /mnt/m1nsh1m /usr/local
if mount -o ro /dev/disk/by-label/M1NSH1M /mnt/m1nsh1m >/dev/null 2>&1; then
	mount --bind /mnt/m1nsh1m/chromebrew /usr/local >/dev/null 2>&1 || :
fi
...
while :; do
	...
	read -rep "> " choice
	case "$choice" in
	[pP]) /usr/sbin/m1nsh1m_payload.sh ;;
	[bB]) run_task bash ;;
